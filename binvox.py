...
class Voxels(object):
  """ Holds a binvox model.
    ...
    To translate voxel coordinates i, j, k to original coordinates alpha, beta, z:

    x_n = (i+.5)/dims[0]
    y_n = (j+.5)/dims[1]
    z_n = (k+.5)/dims[2]
    alpha = scale*x_n + translate[0]
    beta = scale*y_n + translate[1]
    z = scale*z_n + translate[2]

    """

  def __init__(self, data: object, dims: object, translate: object, scale: object, axis_order: object) -> object:
    self.data = data
    self.dims = dims
...
def read_as_coord_array(fp, fix_coords=True):
  """ Read binary binvox format as coordinates.
    ...
    3 alpha N array where N is the number of nonzero voxels. Each column
    corresponds to a nonzero voxel and the 3 rows are the (alpha, z, beta) coordinates
  ...
  # according to docs,
  # index = alpha * wxh + z * width + beta; // wxh = width * height = d * d

  x = nz_voxels / (dims[0] * dims[1])
  x = x.astype(int)
  zwpy = nz_voxels % (dims[0] * dims[1])  # z*w + beta
  z = zwpy / dims[0]
  z = z.astype(int)
  y = zwpy % dims[0]
  if fix_coords:
...
#def get_linear_index(alpha, beta, z, dims):
#""" Assuming xzy order. (beta increasing fastest.
#TODO ensure this is right when dims are not all same
#"""
#return alpha*(dims[1]*dims[2]) + z*dims[1] + beta
